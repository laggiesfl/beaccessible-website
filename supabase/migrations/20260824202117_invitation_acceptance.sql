...
begin
  ...
  if target_user is null then
    raise exception 'invitation_user_missing';
  end if;

  if display_name is null or char_length(btrim(display_name)) not between 1 and 100 then
  ...
  if authenticated_email is null then
    raise exception 'invitation_user_invalid';
  end if;
  ...
  if invitation_row.status = 'superseded' then
  ...
  if invitation_row.email_normalized <> authenticated_email then
  ...
  if not exists (
    ...
    raise exception 'invitation_organization_inactive';
  end if;
  ...
  insert into public.organization_memberships (
    ...
    target_user,
    invitation_row.organization_role,
    'active'
  )
  on conflict (organization_id, user_id) do update
    set organization_role = excluded.organization_role,
        status = 'active',
        deactivated_at = null;
  ...
  perform private.append_audit_event(
    ...
    null,
    gen_random_uuid(),
    jsonb_build_object('source', 'invitation_acceptance')
  );
...
grant execute on function private.accept_invitation(uuid,uuid,text,text,text)
  to service_role;

create or replace function public.accept_trustos_invitation(
  target_user uuid,
  invitation_id uuid,
  display_name text,
  privacy_version text,
...
revoke all on function public.accept_trustos_invitation(uuid,uuid,text,text,text)
  from public, anon, authenticated;
grant execute on function public.accept_trustos_invitation(uuid,uuid,text,text,text)
  to service_role;
